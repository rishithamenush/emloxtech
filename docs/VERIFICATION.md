...
## Published product and contact update

Money Maker Google Play listing verified: https://play.google.com/store/apps/details?id=app.moneymaker.android. Added published-product and mobile-service pages plus website privacy notice; 19 indexable pages, 23 prerendered routes including legacy redirects and 404. Production build and SEO checks pass. Mocked contact API tests cover validation, origin checks, fixed recipient/reply-to, provider failure, retries, and throttling. Browser preview verified Money Maker layout and contact review/email fallback without sending mail. Social preview PNG visually inspected at 1200×630. Vercel headers configured but not verified on the live host. Direct email delivery requires provider configuration and an actual inbox test; it is not active solely from this code update.
